...
def _extract_structured_data(content: str, query: str) -> dict:
    ...
    """
    logger.info(f"Extracting structured data for: {query}")
    ...
    try:
        ...
        logger.info(f"Successfully extracted structured data for: {query}")
        return data
...
def search_and_extract_web_data(query: str, preferred_domains: list = None) -> dict:
    ...
    """
    if not exa:
        logger.warning("EXA_API_KEY not available. Skipping web search.")
        return {}
        
    try:
        logger.info(f"Performing Exa web search for: {query}")
        ...
        if preferred_domains:
            for dom in preferred_domains:
                ...
                logger.debug(f"Trying site-restricted search: {try_query}")
                sr = exa.search(query=try_query, type="neural", num_results=3)
                if sr and getattr(sr, 'results', None):
                    search_response = sr
                    logger.info(f"Site-restricted search for {dom} returned {len(sr.results)} results")
                    break
        ...
        if not result_urls:
            logger.warning("No result URLs found in search results.")
            return {}
        ...
        if not getattr(contents_response, 'results', None):
            logger.warning("No contents returned from Exa get_contents.")
            return {}
        ...
        if hasattr(top_result, 'text') and top_result.text:
            ...
            logger.warning("No text content available from search result.")
            return {}
        ...
        logger.exception(f"Error during Exa search and extraction: {e}")
        return {}
